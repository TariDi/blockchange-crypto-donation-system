      ...
      <p-tab-view>
        <p-tab-panel header="Donor Account">
          <div class="login-tab">
            <div class="login-details">
              ...
              <p-input-text type="text" v-model="value" placeholder="Username" />
              </div>
              <div class="login-password">
              <p-password v-model="donorPassword" placeholder="Password" :feedback="false" toggleMask/>
              </div>
            </div>
            <div>
              <p-button type="submit" label="Submit" :loading="submitLoading" @click="attemptLoginDonor"/>
            </div>  
          </div> 
        </p-tab-panel>
        <p-tab-panel header="Beneficiary Account" @click="toggleTheme">
          <div class="login-tab">
            ...
            <div>
              <p-button type="submit" label="Submit" :loading="submitLoading" @click="attemptLoginBeneficiary"/>
            </div>  
          </div> 
        </p-tab-panel>
      </p-tab-view>
...
// change current theme to next
    toggleTheme() {
        ...
        else if (this.currentTheme.value === 'viva-light') nextTheme = 'lara-light-teal'

        // 1. Current theme name
        // 2. Next theme name
        // 3. id of <link>, what reference to where set theme css file --> fix, single id to <link>
        this.PrimeVue.changeTheme(this.currentTheme.value, nextTheme, 'theme-id-link', () => {});    
